...
class CuentaBancariav2 {
    ...
    public void retirar(int cantidad, int clienteId) {
        ...
        // Entrar a la sección crítica usando la interrupción forzada de Dekker
        while (true) {
            bandera[clienteId - 1] = true; // Indica que el cliente está listo para entrar
            // Verifica si el otro cliente está listo y si es su turno
            ...
            // Si el otro cliente está listo y no es su turno, se da un "yield" para permitir que el otro hilo acceda
            Thread.yield(); // Permite que el otro hilo ejecute su código
        }
    }
}

// Clase que representa un cliente que intenta retirar dinero
class Clientev2 extends Thread {
    private final CuentaBancariav2 cuenta; // Referencia a la cuenta bancaria
    private final int cantidad;
    private final int id;

    public Clientev2(CuentaBancariav2 cuenta, int cantidad, int id) {
        this.cuenta = cuenta;
